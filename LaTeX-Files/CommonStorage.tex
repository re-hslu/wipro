\subsubsection{Storage}  \label{sec:commonstorage}

Die Klasse \texttt{CommonStorageService} stellt die zentrale Speicherschicht der Anwendung dar und bietet eine einheitliche Schnittstelle für persistente Datenspeicherung. 
...
\hspace{0.05cm}
\begin{lstlisting}[language=Kotlin, caption={AppDatabase}]
@Database(
...
\hspace{0.05cm}
\begin{lstlisting}[language=Kotlin, caption={Beispiel JSON-Speicher}]
...
Die \newline \texttt{getModuleData()}-Methode lädt Daten aus der Datenbank und gibt sie als JSON-String zurück.
...
\hspace{0.05cm}
\begin{lstlisting}[language=Kotlin, caption={Beispiel File-Speicher}]
