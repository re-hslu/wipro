...
Android-SDK-Level~36 \parencite{google_android-releases_nodate}.
...
empfohlene Standard \parencite{google_sdk_nodate}. Ziel war es, wieder auf dem
...
(\texttt{hslui} und \texttt{hsluta}) realisiert \parencite{google_build-varianten_nodate}. Tenant-spezifische Konfigurationen
wie Tenant-ID, Name oder API-Tokens werden dabei direkt im
\newline \texttt{build.gradle} pro Flavor definiert. Zusätzlich existiert für jeden
...
Hauptmanifest und das jeweilige Flavor-Manifest zusammengeführt \parencite{google_manage_nodate}. Dabei wird der
Einstiegspunkt der App tenant-spezifisch gesetzt: Für \texttt{hslui} wird die
\newline \texttt{HsluIApplication} instanziiert, für \texttt{hsluta} entsprechend die
...
\newline \texttt{CommonNetworkService} (Abschnitt~\ref{sec:commonnetwork}) und 
\texttt{CommonStorageService} (Abschnitt~\ref{sec:commonstorage}) erzeugt.
Diese werden als Singletons bereitgestellt, da sie während der gesamten Laufzeit der App nur einmal existieren sollen.
...
\begin{lstlisting}[language=Kotlin, caption={Provider-Methode}]
@Provides
...
\begin{lstlisting}[language=Kotlin, caption={Injection im NewsDataLoader}]
@HiltViewModel
