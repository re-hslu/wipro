...
Das Modul CommonDomain stellt die Domänenschicht der Anwendung dar und enthält alle zentralen Geschäftsobjekte, 
...
\hspace{0.05cm}
\begin{lstlisting}[language=Kotlin, caption={Klasse CommonAppBaseModuleItemAPIDTO}]
abstract class CommonAppBaseModuleItemAPIDTO {
...
\hspace{0.05cm}
\begin{lstlisting}[language=Kotlin, caption={Klasse CommonAppBaseModuleAPIDTO}]
data class CommonAppBaseModuleAPIDTO<I>(
    val items: List<I>? = null,
    val item: I? = null,
    val ModuleLastUpdated: String? = null
)
\end{lstlisting}
...
Die Verwendung von \newline Gson-Annotationen wie \texttt{@SerializedName} ermöglicht die korrekte Deserialisierung von JSON-Daten aus der API.
...
\hspace{0.05cm}
\begin{lstlisting}[language=Kotlin, caption={Klasse CommonModuleLoaderStatus}]
sealed class CommonModuleLoaderStatus<I> {
    object Not_Initialized : CommonModuleLoaderStatus<Nothing>()
    data class Success<I>(val data: I) : CommonModuleLoaderStatus<I>()
    // ...
}
\end{lstlisting}

\texttt{CommonAppModuleType} definiert alle verfügbaren Modultypen der Anwendung und bietet eine \texttt{parse()}-Methode zur Konvertierung von Strings.

Die Konfigurationsklassen definieren die Einstellungen für verschiedene Module. 
\newline \texttt{CommonModuleBaseLoaderConfig} stellt die Basis-Konfiguration für Modul-Loader bereit.

Das \texttt{ModuleLoaderContract} Interface definiert den Vertrag für alle Modul-Loader und stellt die grundlegenden Methoden wie \texttt{setup()} und \texttt{process()} bereit.
...
Die Implementierung verwendet \texttt{Locale.getDefault().language} und einen \texttt{when}-Ausdruck, um zwischen deutschen und englischen Titeln zu wählen. \\
\hline
