...
Das Modul CommonApplication stellt gemeinsame Basisklassen für das Laden 
und Synchronisieren der Daten von Features (z.B. News, Blog,...) bereit. Das Herzstück ist dabei die Klasse 
\texttt{CommonApplicationBaseModuleLoader}, die Feature-Daten aus dem lokalen Speicher und der Backend-API liest/speichert. 
(Zusätzliches Diagramm dazu im Anhang ~\ref{anhang:CommonApplicationBaseModuleLoader}).
...
Durch eine abstrakte Basisklasse \newline \texttt{CommonApplicationBaseModuleLoader}, die den Datenlade- und Synchronisationsprozess bereitstellt, 
müssen konkrete Feature-Module (z.B. News, Blog,...) nur noch ihre spezifischen Typparameter und Konfigurationen bereitstellen. 
Dies reduziert den Implementierungsaufwand erheblich und stellt sicher, dass alle Module den gleichen Ladevorgang einhalten.
...
Die Klassendefinition sieht folgendermassen aus:

\hspace{0.05cm}
\begin{lstlisting}[language=Kotlin, caption={Klassendefinition CommonApplicationBaseModuleLoader}]
abstract class CommonApplicationBaseModuleLoader<I, T, C>(
    // ...
) : ViewModel(), ModuleLoaderContract<I>
\end{lstlisting}
...
Die Methode prüft zunächst die Netzwerkverbindung und wählt dann zwischen Remote-, Cache- oder Hybrid-Modus.
...
\hspace{0.05cm}
\begin{lstlisting}[language=Kotlin, caption={Timestamp-Vergleich in CommonApplicationBaseModuleLoader}]
...
\hspace{0.05cm}
\begin{lstlisting}[language=Kotlin, caption={Deserialisierung mit GSON}]
...
Die Klasse wird mit der \texttt{@HiltViewModel}-Annotation versehen und erbt von der Basisklasse. \\
\hline
