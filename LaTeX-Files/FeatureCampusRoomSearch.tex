...
einschließlich aller PDF-Dateien.
...
Die vollständige Implementierung der \texttt{process()}-Methode findet sich in \texttt{CampusRoomSearchModuleLoader.kt}, Zeilen 80-100.
...
Die vollständige Implementierung der \texttt{loadRemoteDetailedData()}-Methode findet sich in \texttt{CampusRoomSearchModuleLoader.kt}, Zeilen 102-150.

Die \texttt{PDFRoomSearchView} Komponente rendert PDF-Dateien mit Android's \texttt{PdfRenderer} und unterstützt interak
tive Annotationen. Die Komponente lädt PDFs aus der Room-Datenbank und ermöglicht Zoom- und Pan-Gesten.
Die vollständige Implementierung der \texttt{PDFRoomSearchView} Komponente findet sich in \texttt{PDFRoomSearchView.kt}, Zeilen 38-60.
...
Die vollständige Implementierung der \texttt{transformTapCoordinates()}-Methode findet sich in \texttt{PDFRoomSearchView.kt}, Zeilen 147-165.
...
Die vollständige Implementierung des \texttt{IJavascriptHandler} findet sich in \texttt{RoomsearchViewModel.kt}, Zeilen 231-242.

\subsubsection*{Spezifische Infos}

Die folgende Tabelle gibt einen Überblick über die wichtigsten Funktionalitäten des \texttt{CampusRoomSearch} Features:
